...
class AuthController {
  async authStaff(req, res) {
    ...
    try {
      ...
      const accessToken = jwt.sign(user, process.env.ACCESS_TOKEN_SECRET, { expiresIn: '10s' });
      const refreshToken = jwt.sign(user, process.env.REFRESH_TOKEN_SECRET);
      const ipRefreshToken = { ...JSON.parse(staff.refresh_token), [String(ipAddress)]: String(refreshToken)};
      ...
      const dataToQuery = [JSON.stringify(ipRefreshToken), String(dateTime), JSON.stringify(newHistory), JSON.stringify(devices), username];
      ...
      // res.cookie('accessToken', accessToken, { httpOnly: true, secure: true, sameSite: 'None' });
      // res.cookie('refreshToken', refreshToken, { httpOnly: true, secure: true, sameSite: 'None' });
      console.log(staff.staff_accessibility);
  ...
  async authToken(req, res) {
    let connection;
    try {
      const { token, staff_username, ipAddress } = req.body;
      if (staff_username === process.env.DEVELOPER_USERNAME) {
        jwt.verify(token, process.env.REFRESH_TOKEN_SECRET, (err, decoded) => {
          if (err) return res.status(403).json({ status: 403, err });
  
          const accessToken = jwt.sign({ staff_username: decoded.staff_username, role: decoded.role }, process.env.ACCESS_TOKEN_SECRET, { expiresIn: '1hr' });
        ...
        // console.log(JSON.parse(user.refresh_token)[ipAddress]);
        console.log(JSON.parse(user.refresh_token)[ipAddress], token);
        console.log(JSON.parse(user.refresh_token)[ipAddress] !== token);
  
        if (JSON.parse(user.refresh_token)[ipAddress] !== token) return res.status(403).json({ status: 403, message: 'Token did not match!' });
  
        jwt.verify(token, process.env.REFRESH_TOKEN_SECRET, (err, decoded) => {
          if (err) return res.status(403).json({ status: 403, err });
  
          const accessToken = jwt.sign({ staff_username: decoded.staff_username, role: decoded.role, accessibility: decoded.staff_accessibility }, process.env.ACCESS_TOKEN_SECRET, { expiresIn: '1hr' });
